...
graph TB
    ...
    B --> B1[주변 사람과 가까워지기 어렵다]
    B --> B2[동료와 성격이 안 맞는다]

    C --> C1[국적 취득이 힘들다]
    C --> C2[한국어 실력이 제자리걸음이다]

    D --> D1[자녀와 육아를 병행하기 힘들다]
    D --> D2[학부모 모임에 참여하기 두렵다]
...
```mermaid
flowchart LR
    A[Động từ gốc] --> B{Có patchim?}
    B -->|Có| C[+ 으려던 참이다]
    B -->|Không| D[+ 려던 참이다]
    B -->|Kết thúc bằng ㄹ| E[Bỏ ㄹ + 려던 참이다]
...
```mermaid
flowchart TD
    A[Có nỗi lo] --> B[고민을 털어놓다<br/>Tâm sự nỗi lo]
    B --> C[상담을 받다<br/>Nhận tư vấn]
    C --> D[조언을 구하다<br/>Xin lời khuyên]
    D --> E[고민을 해결하다<br/>Giải quyết nỗi lo]
    E --> F[고민을 덜다<br/>Giảm bớt lo lắng]
```
