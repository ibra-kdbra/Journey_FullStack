...
> A plain-text design system contract. Read this file before building UI anywhere in
> the repository, so every surface matches the others instead of inventing a new look
> per project.
>
...
repository. A component that "looks fine" in one theme at one width is not
finished. These are the boring failures that ship most often.

### Blocking

1. **No hardcoded colours.** Every colour resolves through a CSS custom property from
   [`learning-doc/assets/css/courses.css`](learning-doc/assets/css/courses.css). A
   literal hex, `rgb()`, or named colour in a component is a blocker.
2. **Both themes, always.** A rule that only appears under `.dark` without a light
...
4. **Contrast floors.** Body text ≥7:1, large text and UI chrome ≥4.5:1, non-text
   boundaries ≥3:1. `--color-text-muted` never carries body copy.
5. **State is never colour alone.** Active nav, validation and status pair colour
   with an icon, weight change, or indicator.
...
# Literal colours — any hit outside var(--…) is a blocker
...
Then render the change in light and dark at 375px, 768px and 1440px.
...
Zero blocking findings is stated explicitly, with what was checked. Silence is not
a pass.
