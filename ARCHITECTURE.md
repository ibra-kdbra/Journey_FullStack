...
| **Vue 3** | [`vue3-clean-architecture`](vue3-clean-architecture/) | The same layering in plain Vue 3 + Vite, no meta-framework |
| **Nuxt** | [`nuxt-clean-architecture`](nuxt-clean-architecture/) | Six-stage refactor, naive component → use case behind a port |

---

## Reviewing a change

The question to ask before "does this work?" is **"which layer owns this, and what
is it allowed to know?"** Dependency direction is the one mistake that is expensive
to undo later, and a single inward-pointing import is how a clean codebase rots.

Read the layer boundary from the filesystem, not from intentions stated in a pull
request description.
...
1. **The dependency rule is absolute.** Source-code dependencies point inward only:
   ...
   the outer rings — no ORM, no HTTP framework, no DTO shaped by transport.
2. **Frameworks live at the edge.** `express`, `@nestjs/*`, `next`, `nuxt`,
   `sequelize`, `typeorm`, `drizzle`, `prisma` and `pocketbase` may not appear under
   a `domain/` or `entities/` path.
3. **Interfaces are declared by the consumer.** A repository interface belongs beside
   the use case that needs it, not beside the implementation that satisfies it. That
   is Dependency Inversion; anything else is folder decoration.
4. **A use case does one thing.** If a class name contains "And", or the file has more
   than one public entry point, split it.
5. **In-memory doubles are part of the design.** Every repository interface has an
   in-memory implementation used by tests. If a use case cannot be tested without a
   database, the abstraction failed.
6. **Cross-project consistency matters.** The Vue, Nuxt and React Native projects
   should name the same concept the same way. Divergence needs a reason recorded in
   [`docs/decisions/`](docs/decisions/).

### Mechanical checks

Group the changed files by project, then for each one:

```bash
# 1. Framework imports inside an inner layer — any hit is a finding.
grep -rnE "from ['\"](express|@nestjs|next|nuxt|sequelize|typeorm|drizzle-orm|@prisma|axios)" \
  --include='*.ts' --include='*.tsx' --include='*.vue' \
  <project>/src/domain <project>/src/entities 2>/dev/null

# 2. Every new port has an in-memory implementation.
ls <project>/src/infrastructure/in-memory/
```

Then read each new interface and confirm it sits in the layer that *consumes* it,
and that each new use case has a test driven by an in-memory double.

### Reporting

Order findings by blast radius — dependency-direction violations first, naming and
structure last. Every finding names a file and a line:

```
<file>:<line> — <rule violated>
  Why it matters: <the concrete failure this causes later>
  Fix: <the specific move, rename, or extraction>
```

No "consider refactoring" without naming the target shape.
