...
```mermaid
graph LR
    A[Reason 1<br/>바쁘다<br/>busy] --> B{-고 해서}
    C[Reason 2<br/>피곤하다<br/>tired] --> B
    D[Reason 3<br/>시간이 없다<br/>no time] --> B
    B --> E[Result<br/>못 만났어요<br/>couldn't meet]
...
```mermaid
flowchart TB
    A[대인 관계<br/>Interpersonal Relationships] --> B[친구 관계<br/>Friendships]
    A --> C[선후배 관계<br/>Senior-Junior Relationships]
    A --> D[직장 관계<br/>Workplace Relationships]

    B --> B1[공감대가 있다<br/>have common ground]
    B --> B2[자주 연락하다<br/>keep in touch often]

    C --> C1[선배를 대하기 어렵다<br/>hard to face seniors]
    C --> C2[예의를 지키다<br/>maintain etiquette]

    D --> D1[상사의 지시<br/>superior's orders]
    D --> D2[동료와 협력<br/>cooperate with colleagues]
...
```mermaid
sequenceDiagram
    participant A as New Environment<br/>(새 환경)
    participant B as Adaptation Process<br/>(적응 과정)
    participant C as Success<br/>(성공)

    A->>B: Finding common ground<br/>(공감대 찾기)
    B->>B: Keeping in touch often<br/>(자주 연락하기)
    B->>B: Maintaining etiquette<br/>(예의 지키기)
    B->>C: Relationship formation<br/>(관계 형성)
```
